...
#### no need specific, just follow system's path
...
(
    ...
    sed -e '/\/unittest\//d' -e '/\/build\//d' -e '/\/CVS\//d' -e '/\/RCS\//d' -e '/\/.git\//d' -e '/\/.hg\//d' -e 's/^\.\///' | \
    sort > ${LIST_FILE}
...
ctags --append=no --links=no -e -f ${DATABASE_FILE} -L ${LIST_FILE}
